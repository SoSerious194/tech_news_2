class AppStrings {
  static String appName = "Tech News";

  // Get started screen
  static String getStarted = "Get Started";
  static String getStartedTitle = "Welcome!";
  static String getStartedDescription = "Explore and follow topics and stay up-to-date with latest in tech!";
  static String loginError = "Network error! Please try again";

  // Home page
  static String homeItem = "Home";
  static String savedItem = "Saved";
  static String settingsItem = "Settings";

  // News page
  static String reviewRadar = "Review Radar";
  static String gadgets360 = "Gadgets360";
  static String theVerge = "The Verge";
  static String techCrunch = "Tech Crunch";
  static String fpt = "Front Page Tech";
  static String liveMint = "Live Mint";
  static String noNews = "No News Available!";
  static String newsError = "Error fetching the latest news";

  // News detail page
  static String article = "Article";
  static String by = "by";

  // General string
  static String cancel = "Cancel";
  static String error = "Error!";
  static String okButton = "OK";
}